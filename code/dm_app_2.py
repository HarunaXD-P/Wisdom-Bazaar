...
import json
from init import db
from dm_database_2 import User, Product, History, Wanted, Correlation
...
def Email_exist():
    ...
    if email_cnt != 0:
        user_x = User.query.filter(User.email == info["user_name"]).first()
        user_dict = to_dict(user_x)
        user_dict["result"] = "success"
        res = make_response(json.dumps(user_dict))
        res.headers["Access-Control-Allow-Origin"] = '*'
