...
import json
from init import db
from dm_database_1 import User, Product, History
...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:123456@127.0.0.1:3306/test_1'
...
def buy_product():
    ...
    # register
        ...
        product_x = Product.query.filter(Product.product_id == info["sold_product_id"]).first()
        print(product_x)
        ...
        Product.query.filter(Product.product_id == info["sold_product_id"]).first().availability_state = 2
        db.session.commit()
